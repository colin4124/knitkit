...
// Mutable global state for knitkit that can appear outside a Builder context
class knitkitContext() {
  val idGen = new IdGen
}
...
object Builder {
  ...
  private val knitkitContext = new DynamicVariable[knitkitContext](new knitkitContext)

  def idGen: IdGen = knitkitContext.value.idGen
  ...
  def forcedUserModule: RawModule = currentModule match {
    ...
    case _ => throwException(
      "Error: Not in a UserModule. Likely cause: Missed Module() wrap, bare knitkit API call, or attempting to construct hardware inside a BlackBox."
    )
  }
  ...
  def build[T <: RawModule](f: => T): (Circuit, T) = {
    knitkitContext.withValue(new knitkitContext) {
  ...
  def error(str: String, cause: Throwable = null) = throw new knitkitException(str, cause)
  ...
  def nameRecursively(prefix: String, nameMe: Any, namer: (HasId, String) => Unit): Unit = nameMe match {
    ...
    case (iter: Iterable[_]) if (iter.knownSize >= 0) =>
